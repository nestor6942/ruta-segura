 ...
 * Stripe, Mercado Pago, SPEI (STP) y OXXO Pay ($120 MXN / cada 2 meses)
 */
...
class PaymentService {
  ...
  async createStripeSession({ telefono, nombre, correo, refCode, hostUrl }) {
    ...
    if (stripeKey && !stripeKey.includes('mock') && stripeKey.startsWith('sk_')) {
      try {
        ...
        bodyParams.append('line_items[0][price_data][product_data][name]', 'Ruta Segura — Suscripción Bimestral ($120 MXN)');
    ...
    if (mpToken && !mpToken.includes('mock') && mpToken.startsWith('APP_USR')) {
      try {
        const payload = {
          items: [
            {
              title: 'Ruta Segura — Cobertura Bimestral ($120 MXN)',
  ...
  completarPago({
    ...
    fechaExp.setMonth(fechaExp.getMonth() + 2); // 2 meses de vigencia (bimestral)
    ...
    if (!usuario) {
      ...
      usuario.plan = 'Bimestral Premium ($120 MXN)';
